"""Diagnostic trials for a DERA program."""
...
from schema import DiagnosticSummary, SearchSpace, TrialRecord
...
class DiagnosticRunner:
    ...
    def config(self, program_dir: Path, space: SearchSpace) -> dict[str, Any]:
        """Read the program's inherited values for the active search space."""
        result: dict[str, Any] = {}
        constants_by_file: dict[str, dict[str, Any]] = {}
        for parameter in space.parameters:
            if parameter.file not in constants_by_file:
                constants_by_file[parameter.file] = _literal_constants(program_dir / parameter.file)
            constants = constants_by_file[parameter.file]
            result[parameter.name] = constants[parameter.name]
        return result

    def materialize(self, program_dir: Path, space: SearchSpace, config: dict[str, Any]) -> None:
        """Apply a measured configuration to the program source tree."""
        for relative in {parameter.file for parameter in space.parameters}:
            ...
            if values:
                _patch_constants(program_dir / relative, values)

    def _evaluate(self, program_dir: Path, config: dict[str, Any], trial_dir: Path, split: str) -> TrialRecord:
        trial_dir.mkdir(parents=True, exist_ok=True)
        program = trial_dir / "program"
        shutil.copytree(program_dir, program, dirs_exist_ok=True)
        for relative in {p.file for p in self._current_space.parameters}:
            ...
            if values: _patch_constants(program / relative, values)
        data_dir = self.data_root / self.task["data_subdir"] / split
        output = trial_dir / "result.json"
        replacements = {"{python}": sys.executable, "{program_dir}": str(program), "{data_dir}": str(data_dir), "{output}": str(output), "{device}": self.device}
    ...
    def run(self, program_dir: Path, space: SearchSpace, budget: int, seed: int, output_dir: Path, anchor: TrialRecord, tpe_settings: dict[str, Any] | None = None) -> DiagnosticSummary:
        if budget < 1: raise ValueError("budget must be positive")
        self._current_space = space
        direction = self.task["objective"]["direction"]
        defaults = self.config(program_dir, space)
        ...
        records = [TrialRecord(0, defaults, anchor.status, anchor.objective, anchor.elapsed_seconds, anchor.error)]
        anchor_added = anchor.status == "success" and anchor.objective is not None
        if anchor_added:
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            record = self._evaluate(program_dir, config, output_dir / f"trial-{trial_id:04d}", "development")
    ...
    def _summarize(self, direction: str, space: SearchSpace, records: list[TrialRecord], output_dir: Path) -> DiagnosticSummary:
        output_dir.mkdir(parents=True, exist_ok=True)
        ...
        initial_objective = next((x.objective for x in records if x.trial_id == 0 and x.status == "success"), None)
        summary = DiagnosticSummary(direction, initial_objective, best.objective if best else None, best.params if best else {}, records, best_so_far, 1 - len(successful) / len(records) if records else 1.0, space)
        (output_dir / "summary.json").write_text(json.dumps(summary.to_dict(), ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        return summary

    def evaluate(self, program_dir: Path, space: SearchSpace, params: dict[str, Any], split: str, output_dir: Path) -> TrialRecord:
        """Evaluate a program and configuration on a named split."""
        self._current_space = space
        return self._evaluate(program_dir, params, output_dir, split)
